	...
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				...
				editor:this,
				title: '上传文件',
				cssRules:"width:700px;height:330px;",
				buttons:[
				{
					className:'edui-okbutton',
					label:'确定',
 ...
 * ```
 */
UE.plugins['files'] = function () {
	var me = this,thePlugins = 'files';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/files.html',
				name:thePlugins,
				editor:this,
				title: '上传文件插件',
				cssRules:"width:700px;height:400px;",
 ...
 * @example
 * ```javascript
 * editor.execCommand( 'helpers');
 * ```
 */
UE.plugins['helpers'] = function () {
	var me = this,thePlugins = 'helpers';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/helpers.html',
				name:thePlugins,
				editor:this,
				title: '文本框',
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /span/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
				'<nobr>帮助: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
				popup.getDom( 'content' ).innerHTML = html;
				popup.anchorEl = el;
				popup.showAnchor( popup.anchorEl );
    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor:this,
                title: '下拉框',
                cssRules:"width:630px;height:420px;",
                buttons:[
                {
                    className:'edui-okbutton',
                    label:'确定',

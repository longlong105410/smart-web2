 ...
 *
 */
public class ExtModelParser extends ModelParser {

	/**
	 * 解析流程定义文件，并将解析后的对象放入模型容器中
	 * @param bytes
	 */
	public static ExtProcessModel parse(byte[] bytes) {
		...
		if(documentBuilder != null) {
			...
			try {
				...
				ExtProcessModel process = new ExtProcessModel();
				...
				process.setOrgId(processE.getAttribute(ExtNodeParser.ATTR_ORG));
				process.setAttachment(processE.getAttribute(ExtNodeParser.ATTR_ATTACHMENT));
				process.setFormId(processE.getAttribute(ExtNodeParser.ATTR_FORM));
				process.setFlowType(processE.getAttribute(ExtNodeParser.ATTR_FLOW_TYPE));
